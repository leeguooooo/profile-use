...
诚实的回答——数据有三条通道，安全程度完全不同：

| 通道 | 它能看到什么 | 结论 |
| --- | --- | --- |
| 辅助脚本（`scripts/profile_use.py`） | 只有本地文件读写。一个文件、零网络代码——你可以自己审计。 | 数据绝不会经脚本离开你的机器。 |
| 模型服务商（Claude / OpenAI / ……） | Agent 读取的值**确实**会进入对话上下文，随 API 请求通过 TLS 发给模型。这是所有 LLM 驱动的自动填表都绕不开的。 | 安全性取决于服务商的 API 数据政策（主流厂商默认不用 API 流量训练）。默认脱敏让会话大部分时间只有打码值；明文按字段、只在填写瞬间读取。 |
| **第三方 API 中转站 / 代理 / 二道贩子** | 每个请求的**完整明文**，包括 agent 读到的任何原始值。 | **别用。** 如果你的 agent 走的中转站是你不敢把身份证递过去的人开的，这个 skill 保护不了你。用官方端点、你自己控制的企业网关，或者本地模型。 |

实用守则：

- 走官方服务商端点：内置的脱敏 + 按字段取明文 + 高敏闸门就是为这个场景设计的。
- 走任何中转站或来路不明的代理：让 agent 填低敏字段，卡号 / 证件号 / 银行信息自己手敲。
- 完全本地的模型（Ollama、llama.cpp 等）：数据根本不出机器——最强配置。
- skill 永远不会把高敏字段批量倒进上下文：`values` 默认排除它们，除非你点名某个字段。一次填表暴露的只有那张表单真正需要的字段。
